...
public class HudHandler {
    public static final ResourceLocation TEX_QS_FAVOURITE = ResourceLocation.fromNamespaceAndPath("morph",
            "textures/gui/fav.png");
    public static final ResourceLocation TEX_QS_SELECTED = ResourceLocation.fromNamespaceAndPath("morph",
            "textures/gui/gui_selected.png");
    public static final ResourceLocation TEX_QS_UNSELECTED = ResourceLocation.fromNamespaceAndPath("morph",
            "textures/gui/gui_unselected.png");
    public static final ResourceLocation TEX_QS_UNSELECTED_SIDE = ResourceLocation.fromNamespaceAndPath("morph",
            "textures/gui/gui_unselected_side.png");
    ...
    public void handleInput(KeyBind keyBind, boolean isReleased) {
        ...
        if (keyBind == KeyBinds.keySelectorUp || keyBind == KeyBinds.keySelectorDown
                || keyBind == KeyBinds.keySelectorLeft || keyBind == KeyBinds.keySelectorRight
...
//        }
    ...
    private void handleMorphInput(KeyBind keyBind, boolean isReleased) {
        if (MorphHandler.INSTANCE.canShowMorphSelector(mc.player)) {
            if (keyBind == KeyBinds.keySelectorDown || keyBind == KeyBinds.keySelectorUp
                    || keyBind == KeyBinds.keySelectorLeft || keyBind == KeyBinds.keySelectorRight) {
                if (showSelector) {
                    shiftIndexSelector(keyBind == KeyBinds.keySelectorDown || keyBind == KeyBinds.keySelectorRight,
                            keyBind == KeyBinds.keySelectorLeft || keyBind == KeyBinds.keySelectorRight);
                } else {
                    showSelector = true;
    ...
    private void updateKeyListeners() {
        ...
        boolean isEnterDown = com.mojang.blaze3d.platform.InputConstants.isKeyDown(handle,
                com.mojang.blaze3d.platform.InputConstants.KEY_RETURN)
                || com.mojang.blaze3d.platform.InputConstants.isKeyDown(handle, GLFW.GLFW_KEY_KP_ENTER);
        boolean isEscDown = com.mojang.blaze3d.platform.InputConstants.isKeyDown(handle,
                com.mojang.blaze3d.platform.InputConstants.KEY_ESCAPE);
        boolean isDeleteDown = com.mojang.blaze3d.platform.InputConstants.isKeyDown(handle,
                com.mojang.blaze3d.platform.InputConstants.KEY_DELETE)
                || com.mojang.blaze3d.platform.InputConstants.isKeyDown(handle, GLFW.GLFW_KEY_KP_DECIMAL);

        boolean isDirUp = com.mojang.blaze3d.platform.InputConstants.isKeyDown(handle,
                com.mojang.blaze3d.platform.InputConstants.KEY_UP);
        boolean isDirDown = com.mojang.blaze3d.platform.InputConstants.isKeyDown(handle,
                com.mojang.blaze3d.platform.InputConstants.KEY_DOWN);
        boolean isDirLeft = com.mojang.blaze3d.platform.InputConstants.isKeyDown(handle,
                com.mojang.blaze3d.platform.InputConstants.KEY_LEFT);
        boolean isDirRight = com.mojang.blaze3d.platform.InputConstants.isKeyDown(handle,
    ...
    private void deleteSelector() {
        if (MorphHandler.INSTANCE.getMorphModeName().equals("classic")) {
            ...
            if (!info.isCurrentlyThisVariant(variant)
                    && !variant.identifier.equals(MorphVariant.IDENTIFIER_DEFAULT_PLAYER_STATE)) // if we're already
                                                                                                 // morphed to this,
                                                                                                 // don't delete it.
                                                                                                 // Also don't delete
                                                                                                 // our default morph
    ...
    private void confirmRadial() {
        if (isMouseOutsideRadialDeadZone(mc.getWindow())) {
            if (radialMode == RadialMode.FAVOURITE) {
                ...
                if (!info.isCurrentlyThisVariant(variant.thisVariant)) // if we're already morphed to this, don't morph
                                                                       // to this.
                {
                    Morph.channel
                            .sendToServer(new PacketMorphInput(variant.thisVariant.identifier, false, false, false));
                }
    ...
    private void toggleFavourite() {
        MorphVariant.Variant variant = getMorphData().morphs.get(indexVert).variants.get(indexHori);
        if (!variant.identifier.equals(MorphVariant.IDENTIFIER_DEFAULT_PLAYER_STATE)) // you can't favourite your
                                                                                      // personal variant
        {
    ...
    private void shiftIndexSelector(boolean isDown, boolean isHori) {
        ...
        if (isDown) {
            ...
            {
                lastIndexHori = (lastIndexHori + (indexHori - lastIndexHori)
                        * (EntityHelper.sineifyProgress(Mth.clamp((float) indexChangeTime / INDEX_TIME, 0F, 1F))));
                ...
                lastIndexVert = (lastIndexVert + (indexVert - lastIndexVert)
                        * (EntityHelper.sineifyProgress(Mth.clamp((float) indexChangeTime / INDEX_TIME, 0F, 1F))));
    ...
    private void drawSelector(GuiGraphics guiGraphics, float partialTick, Window window) {
        ...
        float outProg = EntityHelper
                .sineifyProgress(Mth.clamp((showSelector ? ((showTime + partialTick) / SHOW_SELECTOR_TIME)
                        : (showTime - partialTick) / SHOW_SELECTOR_TIME), 0F, 1F));
        ...
        float indexChangeTimeProg = EntityHelper
                .sineifyProgress(Mth.clamp((indexChangeTime + partialTick) / INDEX_TIME, 0F, 1F));
        ...
        for (int i = 0; i < morphData.morphs.size(); i++) {
            guiGraphics.blit(TEX_QS_UNSELECTED, (int) posX, (int) (top - unSelY + i * size), (int) size, (int) size, 0F,
                    0F, 50, 50, 50, 50);
        }
        ...
        if (width > 0) {
            ...
            for (int i = 0; i < entries; i++) {
                guiGraphics.blit(TEX_QS_UNSELECTED_SIDE, (int) (posX - unSelX + i * size), top, (int) size, (int) size,
                        0F, 0F, 50, 50, 50, 50);
            }
        }

        // Draw the end of the horizontal stack
        guiGraphics.blit(TEX_QS_UNSELECTED, (int) (posX - unSelX + width), top, (int) size, (int) size, 0F, 0F,
                (int) size, (int) size, (int) size, (int) size);

        // Draw the selected marker
        guiGraphics.blit(TEX_QS_SELECTED, (int) posX, top, (int) size, (int) size, 0F, 0F, (int) size, (int) size,
                (int) size, (int) size);

        // Draw the entities
        int screenHeight = window.getGuiScaledHeight();

        int firstMorphIndex = Math.max(0, indexVert - ((int) Math.ceil(top / size) + 1)); // first index to render, +1
                                                                                          // because of the scrolling
        int lastMorphIndex = Math.min(morphData.morphs.size(),
                indexVert + ((int) Math.ceil((screenHeight - top) / size) + 1));
        ...
        for (int i = firstMorphIndex; i < lastMorphIndex; i++) {
            ...
            if (i == indexVert) // is selected
            {
                for (int j = Math.max(0, indexHori - 1); j < morph.variants.size(); j++) {
                    ...
                    if (morphBoxX < window.getGuiScaledWidth() + size) // Only render the entity and the favourite star
                                                                       // if it's at most just barely off screen
                    {
                        EntityDimensions livingSize = living.getDimensions(net.minecraft.world.entity.Pose.STANDING);
                        float targetBoxSize = (float) size * 0.65F;
                        float hScale = targetBoxSize / Math.max(0.1F, livingSize.height());
                        float wScale = targetBoxSize / Math.max(0.1F, livingSize.width());
                        float entScale = Math.min(hScale, wScale);

                        if (j == indexHori) // if it is selected, handle transition.
                        {
                            if (showSelector) {
                                // Grow from 50% to 100% as the selection stabilizes
                                float growthProg = 0.5F + (indexChangeTimeProg * 0.5F);
                                entScale *= growthProg;
                            }
                        } else {
                            entScale *= 0.5F; // Unselected is smaller
                        }

                        // Vertical Center: Align the geometric center of the entity to the center of the box
                        double verticalCenterOffset = size * 0.5D;
                        double entityCenterYOffset = (livingSize.height() / 2.0D) * entScale;
                        double actualMorphHeight = (top + verticalCenterOffset + entityCenterYOffset) + indexSizeHeight;

                        renderMorphEntity(guiGraphics, living, (posX + (size / 2D) - 2) + indexHeightWidth, actualMorphHeight,
                                zLevel + (j == indexHori ? 100F : 50F), entScale);
                        ...
                        if (j == 0 && morph.hasFavourite() || state.variant.thisVariant.isFavourite) {
                            ...
                            guiGraphics.blit(TEX_QS_FAVOURITE, (int) (posX + 1 + indexHeightWidth), (int) favHeight,
                                    (int) (size * 0.15D), (int) (size * 0.15D), 0F, 0F, (int) size, (int) size,
                                    (int) size, (int) size);
                            RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
                    ...
                    if (j == morph.variants.size() - 1) {
                        ...
                        MorphState selectedState = morphStates.computeIfAbsent(selectedVariant,
                                v -> new MorphState(selectedVariant, player));
                        ...
                        EntityType<?> value = net.minecraft.core.registries.BuiltInRegistries.ENTITY_TYPE
                                .get(variant.id);
                        if (value != null) {
                            if (!selectedLiving.getName().getString().equals(value.getDescription().getString())
                                    && showTime >= SHOW_SELECTOR_TIME) // has a custom name
                            {
                        ...
                        if (customName != null) {
                            guiGraphics.drawString(mc.font, customName, 3,
                                    (int) (top + size - mc.font.lineHeight - 5 + indexSizeHeight), 0xFFFFFF);
                        }
                        guiGraphics.pose().popPose();
                    }
                }
                ...
                float currentEntScale = entScale * 0.5F; // Unselected is smaller
                if (i == Math.round(lastIndexVert) && indexChangeTimeProg < 1.0F) // shrinking from selected
                {
                    float shrinkProg = 0.5F + ((1F - indexChangeTimeProg) * 0.5F);
                    currentEntScale = entScale * shrinkProg;
                }

                // Vertical Center: Consistent with the main selector logic
                double verticalCenterOffset = size * 0.5D;
                double entityCenterYOffset = (livingSize.height() / 2.0D) * currentEntScale;
                double actualMorphHeightSidebar = (top + verticalCenterOffset + entityCenterYOffset) + indexSizeHeight;

                if (actualMorphHeightSidebar > top - size && actualMorphHeightSidebar < screenHeight + size) {
                    renderMorphEntity(guiGraphics, living, (posX + (size / 2D) - 2), actualMorphHeightSidebar,
                            zLevel + 50F, currentEntScale);
                }
                ...
                if (morph.hasFavourite()) {
                    ...
                    guiGraphics.blit(TEX_QS_FAVOURITE, (int) posX + 1, (int) favHeight, (int) (size * 0.15D),
                            (int) (size * 0.15D), 0F, 0F, (int) size, (int) size, (int) size, (int) size);
                    RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
                }
    ...
    private void drawRadial(GuiGraphics guiGraphics, float partialTick, Window window) {
        ...
        double diameter = Math.min(window.getGuiScaledWidth(), window.getGuiScaledHeight())
                * Morph.configClient.radialScale;
        ...
        BufferBuilder bufferbuilder = tesselator.begin(VertexFormat.Mode.TRIANGLE_STRIP,
                DefaultVertexFormat.POSITION_COLOR);
        for (int i = 0; i <= slices; i++) {
            double angle = Math.PI * 2 * i / slices;
            bufferbuilder.addVertex(matrix, (float) (Math.cos(angle) * radialDist),
                    (float) (Math.sin(angle) * radialDist), 0).setColor(0, 0, 0, 150);
            bufferbuilder.addVertex(matrix, (float) (Math.cos(angle) * radialDist * deadzoneScale),
                    (float) (Math.sin(angle) * radialDist * deadzoneScale), 0).setColor(0, 0, 0, 150);
        }
        BufferUploader.drawWithShader(bufferbuilder.buildOrThrow());

        MutableComponent catText = Component
                .translatable(radialMode == RadialMode.FAVOURITE ? "morph.key.favourite" : "morph.key.ability");
        ...
        if (radialMode == RadialMode.FAVOURITE) {
            ...
            for (int i = 0; i < radialFavourites.size(); i++) {
                ...
                boolean isSelectedIndex = isMouseOutsideRadialDeadZone(window)
                        && i == MouseHelper.getSelectedIndex(radialFavourites.size());

                EntityDimensions livingSize = living.getDimensions(net.minecraft.world.entity.Pose.STANDING);
                float entSize = Math.max(livingSize.width(), livingSize.height()) / 1.95F; // 1.95F = zombie height

                float entScale = 0.4F * (1F / Math.max(1F, entSize)) * (float) (radialProg * textScale) * 35F; // Base
                                                                                                               // scale
                                                                                                               // 35 as
                                                                                                               // in
                                                                                                               // 1.16
                ...
                renderMorphEntity(guiGraphics, living, -(Math.cos(angle) * radialDist * 0.775F),
                        -(Math.sin(angle) * radialDist * 0.775F), zLevel + 50F, entScale);
                guiGraphics.pose().popPose();
            }
            ...
            for (int i = 0; i < radialFavourites.size(); i++) {
                ...
                guiGraphics.drawString(mc.font, text,
                        (int) ((-(Math.cos(angle) * radialDist * 0.775F) / scale - mc.font.width(text) / 2F)),
                        (int) ((-Math.sin(angle) * radialDist * 0.775F / scale)), 0xFFFFFF);
                guiGraphics.pose().popPose();
            }
        }
    ...
    private boolean bindBiomassBarTexture() {
        if (barAbstractTexture == null && !barTextureGenerated) {
            ...
            Optional<Resource> resource = mc.getResourceManager()
                    .getResource(ResourceLocation.withDefaultNamespace("textures/gui/icons.png"));
                    ...
                    for (int x = 0; x < image.getWidth(); x++) {
                        for (int y = 0; y < image.getHeight(); y++) {
                            ...
                            if ((clr >> 24 & 0xff) > 0) // not invisible
                            {
                                ...
                                int resClr = ((clr >> 24 & 0xff) << 24) | ((desatRgb & 0xff) << 16)
                                        | (((desatRgb >> 8) & 0xff) << 8) | ((desatRgb >> 16) & 0xff);
    ...
    private boolean shouldShowBiomassBar() {
        return Morph.configClient.biomassBarMode == 1 && biomassBarRequiresUpdate()
                || Morph.configClient.biomassBarMode == 2;
    }

    private boolean biomassBarRequiresUpdate() {
        return barCapacity.requiresUpdate() || barCriticalCapacity.requiresUpdate()
                || barCurrentBiomass.requiresUpdate() || barAbilityCost > 0D || barInsufficientFlash > 0;
    }
    ...
    public static void renderMorphEntity(GuiGraphics guiGraphics, LivingEntity living, double x, double y, double z,
            float scale) {
        // Reset entity rotation and walk animation so it shows neutrally in the GUI.
        // Math.PI on Z is intentional - it rotates the entity to face the camera in
        // inventory space.
        ...
        // MCreator and some GeckoLib entities are rotated 180 degrees compared to vanilla.
        // We detect non-vanilla renderers and adjust the rotation so they face the camera.
        float rotation = 180F;
        if (renderer != null && !renderer.getClass().getName().startsWith("net.minecraft.")) {
            rotation = 0F;
        }

        living.setYRot(rotation);
        living.setXRot(0F);
        living.yHeadRot = rotation;
        living.yBodyRot = rotation;
        living.yRotO = rotation;
        living.yHeadRotO = rotation;
        living.yBodyRotO = rotation;
        living.xRotO = 0F;

        // Force visibility and health to ensure all model parts render
        living.setHealth(living.getMaxHealth());
        living.setInvisible(false);
        living.deathTime = 0;
        living.hurtTime = 0;
        living.tickCount = 30; // Use a non-zero tick count to avoid "just spawned" model hiding logic in some mods
        ...
        // Setup environment for GUI entity rendering
        ...
        if (renderer instanceof LivingEntityRenderer) {
            InventoryScreen.renderEntityInInventory(guiGraphics, 0F, 0F, drawScale,
                    new org.joml.Vector3f(), new org.joml.Quaternionf().rotationXYZ(0.43633232F, 0.0F, (float) Math.PI),
                    new org.joml.Quaternionf(), living);
        } else {
            // Fallback: Use our own safe rendering with proper state
            stack.pushPose();
            stack.scale(drawScale, drawScale, drawScale);
            stack.mulPose(new org.joml.Quaternionf().rotationXYZ(0.43633232F, 0.0F, (float) Math.PI));
            MorphRenderHandler.renderLiving(renderer, living, stack, guiGraphics.bufferSource(), 15728880, 0F);
            stack.popPose();
        }
        ...
        // Note: we do NOT restore walkAnimation - the GUI entities should always be
        // idle
    }
    ...
    public void preDrawBiomassBar(GuiGraphics guiGraphics, float partialTick) {
        if (!barRequiresReset && (barShowTime > 0 || shouldShowBiomassBar())) {
            ...
            float prog = EntityHelper.sineifyProgress(Mth
                    .clamp((barShowTime + (shouldShowBiomassBar() ? partialTick : -partialTick)) / BAR_TIME, 0F, 1F));
    ...
    public void drawBiomassBar(GuiGraphics guiGraphics, int x, int y, float partialTick, float prog) {
        if (bindBiomassBarTexture()) // our desat texture could be created
        {
            ...
            if (barInsufficientFlash > 0) {
                float flash = Math
                        .abs((float) Math.cos(Math.toRadians(((barInsufficientFlash - partialTick) / 5F) * 90F)));
                ...
                float criticalRatio = criticalCapacity > 0D ? (float) (capacity / (totalCapacity)) : 1F; // if critical
                                                                                                         // > 0, we
                                                                                                         // should
                                                                                                         // probably
                                                                                                         // draw the
                                                                                                         // crit
                float currentRatio = current <= 0 ? 0F
                        : (float) (current > totalCapacity ? 1F : current / totalCapacity); // capped at 0 minimum
                ...
                if (currentRatio < criticalRatio) // does not exceed critical mass
                {
                    ...
                    if (cost > 0D) // show the cost of the ability
                    {
                        ...
                        {
                            addBiomassBarVertex(bufferbuilder, matrix, x, y, 5, currentRatio,
                                    Math.min(currentRatio + costWidth, 1.0F), r, 0F, 0F, (float) Math.abs(Math.sin(
                                            Math.toRadians(((mc.level.getGameTime() + partialTick) / 10F) * 90F))));
                        } else {
                            addBiomassBarVertex(bufferbuilder, matrix, x, y, 5, currentRatio,
                                    Math.min(currentRatio + costWidth, 1.0F), r, g, b, (float) Math.abs(Math.sin(
                                            Math.toRadians(((mc.level.getGameTime() + partialTick) / 10F) * 90F))));
                        }
                    }
                } else {
                    ...
                    float critPulse = (float) Math
                            .abs(Math.sin(Math.toRadians(((mc.level.getGameTime() + partialTick) / 10F) * 90F)));
                    addBiomassBarVertex(bufferbuilder, matrix, x, y, 5, criticalRatio, currentRatio, r, 0F, 0F,
                            prog * (0.7F + 0.3F * critPulse));
                    ...
                    {
                        float costWidth = totalCapacity <= 0 ? 0F : (float) (cost / totalCapacity);
                        // current ratio is already over critical ratio, we can definitely afford it,
                        // and we'll stay in critical mass, so flash it red
                        addBiomassBarVertex(bufferbuilder, matrix, x, y, 5, currentRatio,
                                Math.min(currentRatio + costWidth, 1.0F), r, 0F, 0F, (float) Math.abs(Math
                                        .sin(Math.toRadians(((mc.level.getGameTime() + partialTick) / 10F) * 90F))));
                    }
                }
            }
            BufferUploader.drawWithShader(bufferbuilder.buildOrThrow());
    ...
    private void addBiomassBarVertex(BufferBuilder bufferbuilder, Matrix4f matrix, int x, int y, int heightOffset,
            float start, float end, float r, float g, float b, float a) {
    ...
    public void onWorldUnload(LevelEvent.Unload event) {
        if (event.getLevel() instanceof net.minecraft.world.level.Level
                && ((net.minecraft.world.level.Level) event.getLevel()).isClientSide()) {
            clean();
        }
    }
