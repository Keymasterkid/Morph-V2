...
//        }
    ...
    private void drawSelector(GuiGraphics guiGraphics, float partialTick, Window window)
    {
        ...
        for (int i = 0; i < morphData.morphs.size(); i++) {
            guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEX_QS_UNSELECTED, (int)posX, (int)(top - unSelY + i * size), 0f, 0f, (int)size, (int)size, 50, 50);
        }
        ...
        if(width > 0)
        {
            ...
            for (int i = 0; i < entries; i++) {
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEX_QS_UNSELECTED_SIDE, (int)(posX - unSelX + i * size), (int)top, 0f, 0f, (int)size, (int)size, 50, 50);
            }
        }

        //Draw the end of the horizontal stack
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEX_QS_UNSELECTED, (int)(posX - unSelX + width), (int)top, 0f, 0f, (int)size, (int)size, (int)size, (int)size);

        //Draw the selected marker
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED, TEX_QS_SELECTED, (int)posX, (int)top, 0f, 0f, (int)size, (int)size, (int)size, (int)size);
        ...
        for(int i = firstMorphIndex; i < lastMorphIndex; i++)
        {
            ...
            if(i == indexVert) //is selected
            {
                for(int j = Math.max(0, indexHori - 1); j < morph.variants.size(); j++)
                {
                    ...
                    if(morphBoxX < window.getGuiScaledWidth() + size) //Only render the entity and the favourite star if it's at most just barely off screen
                    {
                        ...
                        // Vertical Center: Align the geometric center of the entity to the center of
                        // the box
                        ...
                        renderMorphEntity(guiGraphics, living, (posX + (size / 2D) - 2) + indexHeightWidth,
                                actualMorphHeight,
                        ...
                        {
                            guiGraphics.pose().pushMatrix();
                            guiGraphics.pose().translate((float)(0F), (float)(0F));

                            int starColor = state.variant.thisVariant.isFavourite ? net.minecraft.util.ARGB.color(255, 255, 255, 255) : net.minecraft.util.ARGB.color(255, 0, 255, 255);
                            
                            guiGraphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, TEX_QS_FAVOURITE, (int)(posX + 1 + indexHeightWidth), (int)favHeight, 0f, 0f, (int)(size * 0.15D), (int)(size * 0.15D), (int)(size * 0.15D), (int)(size * 0.15D), starColor);
                    ...
                    if(j == morph.variants.size() - 1)
                    {
                        ...
                        EntityType<?> value = net.minecraft.core.registries.BuiltInRegistries.ENTITY_TYPE.get(variant.id).map(net.minecraft.core.Holder::value).orElse(null);
                        if(value != null)
                        {
                            if(!selectedLiving.getName().getString().equals(value.getDescription().getString()) && showTime >= SHOW_SELECTOR_TIME) //has a custom name
                            {
                                ...
                                customName.setStyle(customName.getStyle().withColor(ChatFormatting.WHITE).withItalic(true));
                            }
            ...
            else
            {
                ...
                if(morph.hasFavourite())
                {
                    ...
                    {
                        guiGraphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, TEX_QS_FAVOURITE, (int)posX + 1, (int)favHeight, 0f, 0f, (int)(size * 0.15D), (int)(size * 0.15D), (int)(size * 0.15D), (int)(size * 0.15D));
                    }
                }
    ...
    private void drawRadial(GuiGraphics guiGraphics, float partialTick, Window window)
    {
        ...
        // Render the background donut using pure vanilla guiGraphics batching to bypass entirely specific 1.21 3D translucent shader problems
        int color = net.minecraft.util.ARGB.color(150, 0, 0, 0); // alpha=150, pure black
        int rOuter = (int)radialDist;
        int rInner = (int)(radialDist * deadzoneScale);

        for (int y = -rOuter; y <= rOuter; y++) {
            int xOuter = (int)Math.sqrt(rOuter * rOuter - y * y);
            int xInner = 0;
            if (Math.abs(y) <= rInner) {
                xInner = (int)Math.sqrt(Math.max(0, rInner * rInner - y * y));
            }
            
            // Left segment
            guiGraphics.fill(-xOuter, y, -xInner, y + 1, color);
            // Right segment
            guiGraphics.fill(xInner, y, xOuter, y + 1, color);
        }
        ...
        if(radialMode == RadialMode.FAVOURITE)
        {
            ...
            for(int i = 0; i < radialFavourites.size(); i++)
            {
                ...
                double screenX = centerX - (Math.cos(angle) * radialDist * 0.775F);
                double screenY = centerY + (radialDist * 0.1F) - (Math.sin(angle) * radialDist * 0.775F);
                renderMorphEntity(guiGraphics, living, screenX, screenY, zLevel + 50F, entScale);
            }
            ...
            for(int i = 0; i < radialFavourites.size(); i++)
            {
                ...
                guiGraphics.pose().pushMatrix();
                guiGraphics.pose().translate((float)(0F), (float)(radialDist * 0.1F + mc.font.lineHeight / 1.75F * textScale));
    ...
    private boolean bindBiomassBarTexture()
    {
            ...
            if(resource.isPresent())
            {
                try(InputStream inputStream = resource.get().open())
                {
                    ...
                    for(int x = 0; x < image.getWidth(); x++)
                    {
                        ...
                        {
                            int clr = image.getPixel(x, y); //Actually ABGR in 1.21.1
        ...
        {
            RenderSystem.setShaderTexture(0, barAbstractTexture.getTextureView());
    ...
    public static void renderMorphEntity(GuiGraphics guiGraphics, LivingEntity living, double x, double y, double z, float scale)
    {
        ...
        living.xRotO = 0F;
        living.tickCount = Minecraft.getInstance().player.tickCount;

        // Always show idle stance in selector - set walk animation to stopped
        living.walkAnimation.update(0F, 0F, 1.0F);

        float drawScale = Math.max(scale, 0.01F);
        // Use a large bounding box radius to prevent InventoryScreen's internal scissor from cutting off tall entities (like heads)
        // Since the center (x,y) remains exactly the same, the entity position is unaffected.
        try {
            InventoryScreen.renderEntityInInventory(guiGraphics, (int)(x - 1000), (int)(y - 1000), (int)(x + 1000), (int)(y + 1000), (int)drawScale, new org.joml.Vector3f(), new org.joml.Quaternionf().rotationXYZ(0.43633232F, 0.0F, (float)Math.PI), new org.joml.Quaternionf(), living);
        } catch (Throwable t) {
            // Some modded entities (e.g. GeckoLib) may crash during render state extraction
        }
    ...
    public void drawBiomassBar(GuiGraphics guiGraphics, int x, int y, float partialTick, float prog)
    {
        if(bindBiomassBarTexture()) //our desat texture could be created
        {
            ...
            net.minecraft.client.renderer.MultiBufferSource.BufferSource bufferSourceMC = net.minecraft.client.Minecraft.getInstance().renderBuffers().bufferSource();
            com.mojang.blaze3d.vertex.VertexConsumer vc = bufferSourceMC.getBuffer(
                    net.minecraft.client.renderer.RenderType.entityTranslucent(barAbstractTexture.getResourceLocation()));

            if(barInsufficientFlash > 0)
            {
                float flash = Math.abs((float)Math.cos(Math.toRadians(((barInsufficientFlash - partialTick) / 5F) * 90F)));
                addBiomassBarVertex(vc, matrix, x, y, 0, 0F, 1F, 1F, flash, flash, prog);
            }
            else
            {
                ...
                if(currentRatio < criticalRatio) //does not exceed critical mass
                {
                    ...
                    if(currentRatio > 0F)
                    {
                        addBiomassBarVertex(vc, matrix, x, y, 5, 0F, currentRatio, r, g, b, prog);
                    }

                    //draw the background biomass
                    addBiomassBarVertex(vc, matrix, x, y, 0, currentRatio, criticalRatio, r, g, b, prog);

                    //draw the critical background... if we have any
                    if(criticalRatio < 1F)
                    {
                        addBiomassBarVertex(vc, matrix, x, y, 0, criticalRatio, 1F, r, 0F, 0F, prog);
                    }
                    ...
                    if(cost > 0D) //show the cost of the ability
                    {
                        ...
                        {
                            addBiomassBarVertex(vc, matrix, x, y, 5, currentRatio, Math.min(currentRatio + costWidth, 1.0F), r, 0F, 0F, (float)Math.abs(Math.sin(Math.toRadians(((mc.level.getGameTime() + partialTick) / 10F) * 90F))));
                        }
                        else
                        {
                            addBiomassBarVertex(vc, matrix, x, y, 5, currentRatio, Math.min(currentRatio + costWidth, 1.0F), r, g, b, (float)Math.abs(Math.sin(Math.toRadians(((mc.level.getGameTime() + partialTick) / 10F) * 90F))));
                        }
                    }
                }
                else
                {
                    ...
                    addBiomassBarVertex(vc, matrix, x, y, 5, 0F, criticalRatio, r, g, b, prog);
                    ...
                    addBiomassBarVertex(vc, matrix, x, y, 5, criticalRatio, currentRatio, r, 0F, 0F, prog * (0.7F + 0.3F * critPulse));

                    //draw the critical background
                    addBiomassBarVertex(vc, matrix, x, y, 0, currentRatio, 1F, r, 0F, 0F, prog);
            ...
            // Flush the buffer source so the textured quads actually render
            bufferSourceMC.endBatch();

            guiGraphics.pose().translate((float)(0F), (float)(-6F * prog));
        }
    }

    private void addBiomassBarVertex(com.mojang.blaze3d.vertex.VertexConsumer vc, Matrix4f matrix, int x, int y, int heightOffset, float start, float end, float r, float g, float b, float a)
    {
        ...
        vc.addVertex(matrix, x1, (float)y2, (float)z).setColor(r, g, b, a).setUv(u1, v2);
        vc.addVertex(matrix, x2, (float)y2, (float)z).setColor(r, g, b, a).setUv(u2, v2);
        vc.addVertex(matrix, x2, (float)y , (float)z).setColor(r, g, b, a).setUv(u2, v1);
        vc.addVertex(matrix, x1, (float)y , (float)z).setColor(r, g, b, a).setUv(u1, v1);
    }
