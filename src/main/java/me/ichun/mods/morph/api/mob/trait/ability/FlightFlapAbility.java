...
public class FlightFlapAbility extends Ability<FlightFlapAbility>
{
    ...
    private void combinedTick(float strength, double velocityToAdd)
    {
        ...
        else
        {
            ...
            // serverPlayer.connection.aboveGroundTickCount = 0; // Private access in 1.21.1
        }
    ...
    {
        if(!keyHeld && net.minecraft.client.Minecraft.getInstance().options.keyJump.isDown()) //hit jump key
        {
            ...
            if(!player.getAbilities().flying && canFlap)
            {
                ...
                if (!flag || !(d7 > d8))
                {
                    ...
                    {
                        player.setDeltaMovement(player.getDeltaMovement().add(0.0D, (double)0.04F * player.getAttributeValue(net.neoforged.neoforge.common.NeoForgeMod.SWIM_SPEED), 0.0D));
                    }
                }
                else
                {
        ...
        keyHeld = net.minecraft.client.Minecraft.getInstance().options.keyJump.isDown();
