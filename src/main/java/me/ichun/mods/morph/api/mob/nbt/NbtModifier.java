...
public class NbtModifier
{
    ...
    public void apply(CompoundTag tag) //This is the parent function. This calls all its passed modifiers
    {
    ...
    public static class Modifier
    {
        ...
        {
            //We're told the keep command, InteractionHand the keep and return, do nothing more.
            if(keep != null)
            {
            ...
            if(nestedModifiers != null && !nestedModifiers.isEmpty() && tag.get(key) instanceof CompoundTag)
            {
                HashSet<String> keep = new HashSet<>();
                CompoundTag compoundTag = (CompoundTag)tag.get(key);
                for(Modifier nestedModifier : nestedModifiers)
                {
                    keep.add(nestedModifier.key); //add this key to the ones we wanna keep since we're modifying this
                    nestedModifier.apply(compoundTag);
                }
            ...
            if(nbtValue != null)
            {
                ...
                {
                    MorphApi.getLogger().error("Error applying Modifier key {} of value {}. Incompatible Tag type of {} against {}.", key, value, nbtValue.getType(), ori.getType());
                    return;
                }

                tag.put(key, nbtValue.copy());
            }
        }
