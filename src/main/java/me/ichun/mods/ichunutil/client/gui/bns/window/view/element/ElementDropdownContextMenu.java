        ...
        implements WindowContextMenu.IContextMenu
{
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick)
    {
        ...
        if(renderMinecraftStyle() > 0)
        {
            ...
            int size = 4;
            RenderHelper.draw(guiGraphics, getRight() - size - 3, getTop() + (height / 2d) - size / 2d, size, size, 0, 22D/128D, 33D/128D, 3D/128D, 14D/128D); //down icon
        }
        else
        {
        ...
        if(!text.isEmpty())
        {
            ...
            drawString(guiGraphics, s, getLeft() + 5, getTop() + (height - net.minecraft.client.Minecraft.getInstance().font.lineHeight) / 2F + 1);
        }
    }
