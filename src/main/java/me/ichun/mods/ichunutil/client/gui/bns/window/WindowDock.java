...
public class WindowDock<M extends IWindows> extends Window<M>
{
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick)
    {
        ...
        for(int i = keys.size() - 1; i >= 0; i--)
        {
            ...
            windows.forEach(window -> window.render(guiGraphics, mouseX, mouseY, partialTick));
        }
    }
        ...
        for(Map.Entry<ArrayListHolder, Constraint.Property.Type> e : docked.entrySet())
        {
            if(e.getKey().windows.contains(dockedWin))
            {
                ...
                for(Window<?> window1 : dockStack)
                {
                    ...
                    {
                        window1.resize(net.minecraft.client.Minecraft.getInstance(), this.width, this.height);
                    }
                }
                return true;
            }
    ...
    public void removeFromDock(Window<?> window)
    {
        ...
        window.resize(net.minecraft.client.Minecraft.getInstance(), window.parent.getWidth(), window.parent.getHeight());
