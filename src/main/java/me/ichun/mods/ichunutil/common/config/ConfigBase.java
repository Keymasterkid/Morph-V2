...
//    private final @Nonnull HashSet<String> reveal = new HashSet<>();
    ...
    public <T extends ConfigBase> T init()
    {
        ...
        container.registerConfig(getConfigType(), configBuilder.build());
        // config lookup moved to events
        //End registerConfig

        ModLoadingContext.get().getActiveContainer().getEventBus().addListener(this::onConfigLoad);
        ModLoadingContext.get().getActiveContainer().getEventBus().addListener(this::onConfigReload);
...
//    }
    ...
    public void buildAdditionalConfigs(ModConfigSpec.Builder builder){} //in case any configs aren't fields.
    ...
    private void loadConfig() //massive reflective method here. Sorry cpw!
    {
        ...
        if(getConfigType() != ModConfig.Type.SERVER)
        {
            ...
            {
                Method method = null;
                for (Method m : ConfigTracker.class.getDeclaredMethods()) {
                    if (m.getName().equals("openConfig")) {
                        method = m;
                        break;
                    }
                }
                if (method != null) {
                    ...
                    if (method.getParameterCount() == 2) {
                        method.invoke(ConfigTracker.INSTANCE, config, FMLPaths.CONFIGDIR.get());
                         ...
                         method.invoke(ConfigTracker.INSTANCE, config, FMLPaths.CONFIGDIR.get(), FMLPaths.CONFIGDIR.get().resolve(fileName));
                    }
                } else {
                    iChunUtil.LOGGER.warn("Could not find openConfig method in ConfigTracker via discovery.");
                }
            }
    ...
    private void createWrapper(Field field, ModConfigSpec.Builder builder, String[] lastCat)
    {
...
//        }
    ...
    {
        iChunUtil.LOGGER.info("ConfigEvent onConfigReload fired for file: " + event.getConfig().getFileName());
        if(event.getConfig().getFileName().equals(fileName))
        {
            config = event.getConfig();
            checkForChanges();
    ...
    public void save()
    {
        if(true /* isLoaded removed */) //the child config is set to null on server type configs.
        {
            boolean save = false;
            for(HashSet<ValueWrapper<?>> vals : values.values())
            {
    ...
    public static class ValueWrapper<T>
    {
        ...
        private final @Nonnull ModConfigSpec.ConfigValue<T> configValue;
        public final @Nonnull Field field;

        private T lastObj = null;

        private ValueWrapper(ConfigBase config, ModConfigSpec.ConfigValue<T> configValue, Field field)
        {
            this.parent = config;
