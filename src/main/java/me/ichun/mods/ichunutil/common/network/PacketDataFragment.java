...
public abstract class PacketDataFragment extends AbstractPacket
{
    ...
    {
        // flow().isServerbound() means the packet arrived at the server
        LogicalSide side = context.flow().isServerbound() ? LogicalSide.SERVER : LogicalSide.CLIENT;
        context.enqueueWork(() -> process(side));
    }
